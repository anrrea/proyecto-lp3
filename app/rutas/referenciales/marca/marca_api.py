...
marapi = Blueprint('marapi', __name__)

# Trae todas las marcas
@marapi.route('/marcas', methods=['GET'])
def getMarcas():
    mardao = MarcaDao()

    try:
        marcas = mardao.getMarcas()
...
@marapi.route('/marcas/<int:marca_id>', methods=['GET'])
def getMarca(marca_id):
    mardao = MarcaDao()

    try:
        marca = mardao.getMarcaById(marca_id)
...
@marapi.route('/marcas', methods=['POST'])
def addMarca():
    ...
    try:
        ...
        marca_id = mardao.guardarMarca(descripcion)
...
@marapi.route('/marcas/<int:marca_id>', methods=['PUT'])
def updateMarca(marca_id):
    ...
    try:
        if mardao.updateMarca(marca_id, descripcion.upper()):
            return jsonify({
                'success': True,
                'data': {'id': marca_id, 'descripcion': descripcion},
                'error': None
